...
def search_trials_count(params):
    ...
    with tracer.start_as_current_span("queries.search_trials_count") as span:
        base = 'SELECT COUNT(*) FROM joined_trials'
        where = ['1=1']
        values = []
        ...
        # Note: joins variable is referenced below, ensure correctness in upstream logic
        sql = f"SELECT COUNT(DISTINCT t.id) {' '.join(joins)} WHERE {' AND '.join(where)}"
        span.set_attribute("queries.filters", len(where))
