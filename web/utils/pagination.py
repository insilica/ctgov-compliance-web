...
class Pagination:
    def __init__(self, items, page, per_page, total_entries=None):
        with tracer.start_as_current_span("pagination.Pagination.__init__") as span:
            span.set_attribute("pagination.items_len", len(items) if hasattr(items, "__len__") else 0)
            ...
            span.set_attribute("pagination.total_entries", int(self.total_entries))
            span.set_attribute("pagination.total_pages", int(self.total_pages))
...
def get_pagination_args():
    ...
    with tracer.start_as_current_span("pagination.get_pagination_args") as span:
        try:
        ...
        span.set_attribute("pagination.per_page", per_page)
...
def paginate(items, total_entries=None):
    ...
    """
    with tracer.start_as_current_span("pagination.paginate") as span:
        page, per_page = get_pagination_args()
        pagination = Pagination(items, page, per_page, total_entries)
        span.set_attribute("pagination.total_entries", int(pagination.total_entries))
        span.set_attribute("pagination.page", pagination.page)
        span.set_attribute("pagination.per_page", pagination.per_page)
        span.set_attribute("pagination.total_pages", pagination.total_pages)
