...
def compliance_counts(rates):
    ...
    with tracer.start_as_current_span("route_helpers.compliance_counts") as span:
        # Defer pandas import to function level
        c = rates[0]['compliant_count']
        ic = rates[0]['incompliant_count']
        span.set_attribute("compliance.compliant_count", int(c))
        span.set_attribute("compliance.incompliant_count", int(ic))
        return c, ic
...
def process_index_request(page=None, per_page=None, QueryManager=QueryManager()):
    ...
    with tracer.start_as_current_span("route_helpers.process_index_request") as span:
        # Get pagination parameters from request if not provided
        if page is None or per_page is None:
            page, per_page = get_pagination_args()
        span.set_attribute("pagination.page", int(page))
        span.set_attribute("pagination.per_page", int(per_page))
        ...
        span.set_attribute("trials.total_count", int(total_count))
        ...
        span.set_attribute("compliance.on_time_count", int(on_time_count))
        span.set_attribute("compliance.late_count", int(late_count))
...
def process_search_request(search_params, compliance_status_list, page=None, per_page=None, QueryManager=QueryManager()):
    ...
    with tracer.start_as_current_span("route_helpers.process_search_request") as span:
...
def process_organization_dashboard_request(org_ids, page=None, per_page=None, QueryManager=QueryManager()):
    ...
    with tracer.start_as_current_span("route_helpers.process_organization_dashboard_request") as span:
        ...
        span.set_attribute("org.ids.count", len(org_list))
...
def process_compare_organizations_request(min_compliance, max_compliance, min_trials, max_trials, page=None, per_page=None, QueryManager=QueryManager()):
    ...
    with tracer.start_as_current_span("route_helpers.process_compare_organizations_request") as span:
        # Parse arguments
        ...
        span.set_attribute("filters.min_compliance", parsed_min_compliance if parsed_min_compliance is not None else -1)
        span.set_attribute("filters.max_compliance", parsed_max_compliance if parsed_max_compliance is not None else -1)
        span.set_attribute("filters.min_trials", parsed_min_trials if parsed_min_trials is not None else -1)
        span.set_attribute("filters.max_trials", parsed_max_trials if parsed_max_trials is not None else -1)
        ...
        span.set_attribute("organizations.total_count", int(total_count))
...
def process_user_dashboard_request(user_id, current_user_getter=None, page=None, per_page=None, QueryManager=QueryManager()):
    ...
    with tracer.start_as_current_span("route_helpers.process_user_dashboard_request") as span:
