...
Once dependencies are ready you can boot both the Flask API and the Vite
...
The helper script installs missing frontend dependencies, starts Flask on
`http://localhost:6525`, and launches the Vite dev server on
`http://localhost:5173`, streaming logs with prefixes so you can see which
process emitted each line. Use `CTRL+C` once to stop both services together.
